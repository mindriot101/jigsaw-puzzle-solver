...
def distance_point_line_squared(pos1, origin):
    """
...
def distance_point_line_signed(pos1, origin):
    """
...
def compute_mean_line(lines, debug=False):
    ...
    gaussian_weigthts = np.array(
        [scipy.stats.norm(mean_rho, std_rho).pdf(r) for r in rhos]
    )
...
def compute_intersections(mean_lines, dims):
    h, w = dims
...
def corner_detection(edges, intersections, xs, rect_size=50, show=False):
    ...
    for idx, intersection in enumerate(intersections):
        ...
        if len(edge_idx[0]) > 0:
            distances = [
                (a * edge_x + b * edge_y + c) ** 2 for edge_y, edge_x in zip(*edge_idx)
            ]
...
def compute_line_params(corners):
    return [
        get_line_through_points(corners[i1], corners[i2]) for i1, i2 in _corner_indexes
    ]
...
def shape_classification(edges, line_params, d_threshold=500, n_hs=10):
    ...
    for x_edge, y_edge in zip(x_nonzero, y_nonzero):
        d = [
            distance_point_line_squared(line_param, (x_edge, y_edge))
    ...
    while map_iteration < max_map_iterations:
        ...
        if len(non_classified_points) > 0:
            non_classified_points = np.delete(
                non_classified_points, classified_points_at_current_iteration, axis=0
            )
        else:
            break
...
def compute_inout(class_image, line_params, bs, d_threshold=10):
    ...
    for line_param, cl in zip(line_params, (1, 2, 3, 4)):
        ...
        coords = np.array([list(zip(*np.where(class_image == cl)))])[0]

        distances = np.array(
            [distance_point_line_signed(line_param, (x0, y0)) for y0, x0 in coords]
        )
...
def create_side_images(class_image, inout, corners):
    ...
    for cl in (1, 2, 3, 4):
        ...
        if side_corners[0, 0] != side_corners[1, 0]:
            ...
            correction_angle = -(
                corners_angle / abs(corners_angle) * 90 - corners_angle
            )
...
def get_corners(dst, neighborhood_size=5, score_threshold=0.3, minmax_threshold=100):
    ...
    """
    data = dst.copy()
    data[data < score_threshold * dst.max()] = 0.0
    ...
    def search_for_possible_rectangle(idx, prev_points=[]):
        ...
        if depth == 0:
            right_points_idx = np.nonzero(
                np.logical_and(xy[:, 0] > curr_point[0], distances[idx] > 0)
            )[0]
        ...
        if depth in (1, 2):
            ...
            diff_to_explore = np.nonzero(np.logical_and(all_diffs, distances[idx] > 0))[
                0
            ]
...
def process_piece(image, **kwargs):
    ...
    try:
        ...
        scaled_size = (
            int(gray.shape[0] * params["scale_factor"]),
            int(gray.shape[1] * params["scale_factor"]),
        )
        gray_harris = cv2.resize(gray, scaled_size)

        harris = cv2.cornerHarris(
            gray_harris, params["harris_blocksize"], params["harris_ksize"], 0.04
        )
        ...
        edges = gray - cv2.erode(
            gray, np.ones((params["edge_erode_size"], params["edge_erode_size"]))
        )
        ...
        intersections = np.array(
            np.round([M.dot((point[0], point[1], 1)) for point in intersections])
        ).astype(np.int)
        ...
        corners = corner_detection(
            ...
            intersections,
            (xb, yb),
            params["corner_refine_rect_size"],
            show=False,
        )
        corners = order_corners(corners)
        line_params = compute_line_params(corners)
        class_image = shape_classification(
            edges,
            line_params,
            params["shape_classification_distance_threshold"],
        ...
        inout = compute_inout(
            class_image, line_params, (xb, yb), params["inout_distance_threshold"]
        )
